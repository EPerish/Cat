...
let cats = localStorage.getItem("EPerish-cats");
...
function setLike(id, el) {
    ...
    fetch(`https://cats.petiteweb.dev/api/single/${DBname}/update/${id}`, {
        method: "put",
        headers: {
        ...
        .then(data => {
            ...
            if (data.message.includes("успешно")) {
                ...
                localStorage.setItem("EPerish-cats", JSON.stringify(cats));
            }
        })
}
function setRate(n) {
...
function showModal(id, el) {
    ...
    content.innerHTML = `
        <div class="cat-text">
            ...
            <div>${cat.description || "Информации еще нет..."}</div>
        </div>
...
function setDel(id, el) {
    ...
    fetch(`https://cats.petiteweb.dev/api/single/${DBname}/delete/${id}`, {
        method: "delete"
    })
        .then(res => res.json())
        .then(data => {
...
updForm.addEventListener("submit", (e) => {
    ...
    fetch(`https://cats.petiteweb.dev/api/single/${DBname}/update/${body.id}`, {
        method: "put",
        headers: {
            "Content-Type": "application/json"
        },
...
addForm.addEventListener("submit", (e) => {
    ...
    fetch(`https://cats.petiteweb.dev/api/single/${DBname}/add`, {
        method: "post",
        headers: {
            "Content-Type": "application/json"
        },
